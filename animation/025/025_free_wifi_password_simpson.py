...
from visualizer.simpson_rule import SimpsonRuleVisualizer


class FreeWifiPasswordSimpsonRule(BaseProofScene):
    @override
    def get_title(self) -> str:
        return "An Example of the Simpson's Rule"
    ...
    def after_qed(self) -> None:
        ...
        self.next_section(
            "Calculating the Definite Integration Value by Simpson's Rule")

        simpson_visualizer = SimpsonRuleVisualizer()
        simpson_visualizer.visualize(
            self,
            f,
            (-2, 2),
            iteration_count=6,
