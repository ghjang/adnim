...
class SineRotation(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        # 상단 삼각형
        self.base_unit_circle.upper_triangle.set_points_as_corners(
            triangle_points)

        # 하단 삼각형 (flip 불필요)
        self.base_unit_circle.lower_triangle.set_points_as_corners(
            lower_points)

        # 현 업데이트
        self.base_unit_circle.upper_half_chord.set_points_by_ends(
            circle_point, x_point)
        self.base_unit_circle.lower_half_chord.set_points_by_ends(
        ...
        unit_y = abs(self.base_unit_circle.plane.c2p(0, 1)[1] -
                     self.base_unit_circle.plane.c2p(0, 0)[1])
        min_height = 0.1 * unit_y

        # 기존 브레이스 제거
        if "sine" in self.base_unit_circle.decorations:
            brace, label = self.base_unit_circle.decorations["sine"]
            if brace is not None:
                self.base_unit_circle.remove(brace, label)
                self.base_unit_circle.plane_group.remove_brace(
                    "sine_brace")
            self.base_unit_circle.decorations.pop("sine")
        ...
        if show_brace_for_frame and height > min_height:
            ...
            brace, label = self.base_unit_circle.plane_group.add_brace(
                self.base_unit_circle.upper_half_chord,
                direction=brace_direction,
                name="sine_brace",
                text="\\sin(x)",
                color=StyleConfig.BRACE_COLOR,
                text_color=StyleConfig.BRACE_TEXT_COLOR,
                buff=self.base_unit_circle.base_buff,  # 기본 버퍼 사용
                text_buff=self.base_unit_circle.adjusted_text_buff,  # 조정된 버퍼 사용
            ...
            self.base_unit_circle.decorations["sine"] = (brace, label)
...
class CosineRotation(Animation):
    ...
    def interpolate_mobject(self, alpha):
        ...
        # 우측 삼각형
        self.base_unit_circle.right_triangle.set_points_as_corners(
            triangle_points)

        # 좌측 삼각형
        self.base_unit_circle.left_triangle.set_points_as_corners(
            left_points)

        # 현 업데이트
        self.base_unit_circle.right_half_chord.set_points_by_ends(
            circle_point, y_point)
        self.base_unit_circle.left_half_chord.set_points_by_ends(
        ...
        unit_x = abs(self.base_unit_circle.plane.c2p(1, 0)[0] -
                     self.base_unit_circle.plane.c2p(0, 0)[0])
        min_width = 0.1 * unit_x

        # 기존 브레이스 제거
        if "cosine" in self.base_unit_circle.decorations:
            brace, label = self.base_unit_circle.decorations["cosine"]
            if brace is not None:
                self.base_unit_circle.remove(brace, label)
                self.base_unit_circle.plane_group.remove_brace(
                    "cosine_brace")
            self.base_unit_circle.decorations.pop("cosine")
        ...
        if show_brace_for_frame and width > min_width:
            ...
            # 미리 계산된 폰트 크기 사용
            brace, label = self.base_unit_circle.plane_group.add_brace(
                self.base_unit_circle.right_half_chord,
                direction=brace_direction,
                name="cosine_brace",
                text="\\cos(x)",
                color=StyleConfig.BRACE_COLOR,
            ...
            # VGroup에도 추가
            self.base_unit_circle.add(brace, label)
            self.base_unit_circle.decorations["cosine"] = (brace, label)
