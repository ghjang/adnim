...
class AndGateLearningVisualization(Scene):
    ...
    EQUATION_SCALE = 0.5
    TEXT_COLOR = "#FFA07A"  # Light Salmon color - 수식과 텍스트에 사용할 색상
    TEXT_Z_INDEX = 10  # 텍스트의 z-index 값
    TEXT_LABEL_COLOR = "#FFA07A"  # Light Salmon - 텍스트 레이블 색상
    PARAM_COLORS = {
        "w1": "#FF6B6B",  # 붉은 계열
        "w2": "#4ECDC4",  # 청록 계열
        "b": "#FFD93D",  # 노란 계열
    ...
    def _create_equation_text(
        ...
        if abs(w2) < 1e-10:  # 수직선의 경우
            ...
            # x_val 계산
            x_val = -b / w1
            equation_x = max(min(x_val + 0.5, self.X_RANGE[1] - 1), self.X_RANGE[0] + 1)
        else:
            # y = (-w1/w2)x + (-b/w2) 형태
            equation = MathTex(
                "y = "
            ...
            if abs(slope) < 1e-6:
                ...
                equation_x = max(
                    min(x_at_target_y, self.X_RANGE[1] - 1), self.X_RANGE[0] + 1
                )
