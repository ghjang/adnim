...
class ProofSceneConfig:
    ...
    title_glow_primary: dict = field(
        default_factory=lambda: {"color": "#FFD700", "width": 8, "opacity": 0.15}
    )
    title_glow_secondary: dict = field(
        default_factory=lambda: {"color": "#50C878", "width": 4, "opacity": 0.1}
    )
    title_intro_formula_size: int = 72
    title_display_time: float = 2.0
    title_vertical_offset: np.ndarray = field(default_factory=lambda: DOWN * 0.5)
    ...
    qed_buff: float = 0.2
    qed_shift: np.ndarray = field(default_factory=lambda: UP * 0.25)
    conclusion_animation_time: float = 1.0

    # 레이아웃 설정
    start_position: np.ndarray = field(default_factory=lambda: ORIGIN + DOWN * 1.5)
    scene_end_pause: float = 2.0

    equal_symbol_h_extra_offset: float | np.ndarray = 0
    equal_symbol_h_extra_offset_for_first_step: float | np.ndarray = 0
